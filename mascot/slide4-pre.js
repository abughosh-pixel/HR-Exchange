(() => {
  ...
  const slides = titles.map((title, index) => `<section class="slide mascot-bites-slide" data-title="${title}"><iframe src="../mascot-bites/?bite=${index}" title="HR Exchange Topic ${index + 1}: ${title}"></iframe></section>`).join('');
  original.outerHTML = `${slides}<div id="topicGrid" hidden></div>`;
